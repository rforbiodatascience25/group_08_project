...
comp_de <- function(dat) {   
  dat |> group_by(gene) |> 
    reframe(log2FC = log2(
      mean(expression[condition == "cold"]) / (expression[condition == "control"])),
...
res$neglog10p_capped <- pmin(res$neglog10p, 10)  

ggplot(res_clean, aes(log2FC, neglog10p_capped)) +   
  geom_point(aes(color = sig),       
             alpha = ifelse(res_clean$sig == "Not significant", 0.15, 0.8), 
             size = ifelse(res_clean$sig == "Not significant", 0.5, 1.5)) + 
  ...
  ggtitle("Volcano plot - gene expression in shoots in control vs cold") +   
  labs(
